...
# COLORS
if dark:
    BG="#05080f"; SB_BG="#080c18"; CARD="#0c1220"; BORDER="#1c2e50"
    TEXT="#e2eaf8"; TEXT2="#8aaad4"; TEXT3="#4a78aa"; ACCENT="#3b82f6"; ACCENT2="#6366f1"
    INPUT="#0a1020"; BTN="#0e1c35"; BTN_T="#7aaad8"
    HERO_BG="linear-gradient(135deg,#060d1e,#091428,#050b18)"; HERO_BR="#1c3060"
    ...
    BG="#f0f4ff"; SB_BG="#e2eaf8"; CARD="#ffffff"; BORDER="#c0d0ee"
    TEXT="#0f1a30"; TEXT2="#2a3a5a"; TEXT3="#4a5a7a"; ACCENT="#2563eb"; ACCENT2="#4338ca"
    INPUT="#ffffff"; BTN="#eef2ff"; BTN_T="#2563eb"
    HERO_BG="linear-gradient(135deg,#dce8ff,#eef3ff,#f0ecff)"; HERO_BR="#b0c8f0"
...
html,body,[data-testid="stApp"],[data-testid="stAppViewContainer"]>.main{{background-color:{BG}!important;color:{TEXT}!important;font-family:'Plus Jakarta Sans',sans-serif!important;}}
#MainMenu,footer,header,[data-testid="stToolbar"]{{display:none!important;}}
::-webkit-scrollbar{{width:4px;}}::-webkit-scrollbar-thumb{{background:{BORDER};border-radius:10px;}}
.block-container{{max-width:1080px!important;margin:0 auto!important;padding:1rem 2rem 6rem!important;}}
...
button,[role="button"],label{{cursor:pointer!important;}}
.stButton>button{{background:{BTN}!important;color:{BTN_T}!important;border:1px solid {BORDER}!important;border-radius:10px!important;font-family:'Plus Jakarta Sans',sans-serif!important;font-weight:600!important;font-size:0.85rem!important;padding:0.55rem 1rem!important;transition:all 0.18s!important;}}
...
.stButton>button[kind="primary"]:hover{{box-shadow:0 4px 20px rgba(59,130,246,0.4)!important;}}
.stTextArea textarea,.stTextInput>div>div>input{{background:{INPUT}!important;border:1px solid {BORDER}!important;border-radius:10px!important;color:{TEXT}!important;}}
...
details>summary{{color:{TEXT2}!important;font-family:'Plus Jakarta Sans',sans-serif!important;font-weight:600!important;padding:0.8rem 1rem!important;cursor:pointer!important;}}
...
.stRadio label{{background:{INPUT}!important;border:1px solid {BORDER}!important;border-radius:8px!important;padding:0.4rem 0.9rem!important;color:{TEXT2}!important;font-size:0.85rem!important;cursor:pointer!important;}}
...
.hero-title{{font-size:2.6rem;font-weight:800;line-height:1.15;background:{HTITLE};-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:0.8rem;letter-spacing:-0.02em;}}
.hero-sub{{font-size:1rem;color:{TEXT2};line-height:1.65;max-width:520px;}}
...
def validate_key(key):
    try:
        ...
        r = genai.GenerativeModel("models/gemini-2.5-flash").generate_content("Say OK", generation_config={"max_output_tokens": 5})
...
def generate(prompt, creativity=0.4):
    ...
    for key in keys:
        try:
            genai.configure(api_key=key)
            resp = genai.GenerativeModel("models/gemini-2.5-flash").generate_content(prompt, generation_config={"temperature": creativity, "max_output_tokens": 2000})
...
def run_gen(fmt, content, manual, difficulty, persona, creativity, save_as="output"):
    ...
    if trimmed:
        st.caption("⚠️ Content trimmed to 3000 chars")
    ...
    st.session_state.history.append({"ts": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), "format": fmt, "topic": (manual or "Document")[:50], "output": r})
    return True
...
def show_setup():
    ...
    st.markdown(f"""<div style="max-width:620px;margin:1rem auto;padding:0 1rem;"><div style="text-align:center;padding:1rem 0 1.5rem;"><div style="font-size:2.5rem;">🧠</div><div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:1.5rem;font-weight:800;color:{TEXT};">LearnFlow <span style="color:{ACCENT};">AI</span></div><div style="font-family:'Space Mono',monospace;font-size:0.6rem;color:{TEXT3};letter-spacing:0.15em;margin-top:0.2rem;">STUDY COMPANION</div></div><div class="setup-card"><div class="setup-title">Connect your free AI key</div><div class="setup-sub">LearnFlow AI runs on Google Gemini — 100% free, no credit card needed.<br>Get your key in 2 minutes and start learning instantly.</div><div class="steps-grid"><div class="step-box"><div class="step-n">01</div><div class="step-t">Open Google AI Studio</div></div><div class="step-box"><div class="step-n">02</div><div class="step-t">Sign in with Google</div></div><div class="step-box"><div class="step-n">03</div><div class="step-t">Click Create API Key</div></div><div class="step-box"><div class="step-n">04</div><div class="step-t">Paste below and go!</div></div></div><div class="tip-box">⚡ Keys look like: <strong>AIzaSyAbc123...</strong> (39 characters)</div></div></div>""", unsafe_allow_html=True)

    _, mc, _ = st.columns([1, 2, 1])
    with mc:
        st.markdown(f"""<div style="text-align:center;margin:1.5rem 0 1rem;"><a href="https://aistudio.google.com/app/apikey" target="_blank" style="display:inline-block;background:linear-gradient(135deg,{ACCENT},{ACCENT2});color:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;font-size:0.95rem;padding:0.85rem 2rem;border-radius:12px;text-decoration:none;box-shadow:0 4px 20px rgba(59,130,246,0.3);">🔑 Get My Free API Key →</a></div><div style="text-align:center;font-size:0.76rem;color:{TEXT3};margin-bottom:1.2rem;">Opens Google AI Studio in a new tab</div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""<div class="privacy-box">🔒 <strong>Your privacy:</strong> Key stored only in browser session memory. Never sent to our servers. Auto-deleted when you close this tab.</div><br><div style="font-size:0.78rem;font-weight:700;color:{TEXT2};margin-bottom:0.5rem;">🎓 Everything you unlock:</div><div class="fpills"><span class="fp">📝 Notes</span><span class="fp">🎴 Flashcards</span><span class="fp">❓ Quiz</span><span class="fp">🧪 Feynman</span><span class="fp">🤖 Tutor</span><span class="fp">📅 Study Plan</span><span class="fp">🎓 Exam Mode</span><span class="fp">🧠 Mind Map</span><span class="fp">💡 Mnemonics</span><span class="fp">⚡ TL;DR</span><span class="fp">👶 ELI5</span><span class="fp">⏱ Pomodoro</span><span class="fp">🌙 Dark/Light</span></div>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with lc:
        st.markdown(f"""<div style="padding:0.3rem 0 0.8rem;"><div style="font-size:1rem;font-weight:800;color:{TEXT};font-family:'Plus Jakarta Sans',sans-serif;">🧠 LearnFlow <span style="color:{ACCENT};">AI</span></div><div style="font-family:'Space Mono',monospace;font-size:0.58rem;color:{TEXT3};letter-spacing:0.14em;margin-top:0.2rem;">STUDY COMPANION</div></div>""", unsafe_allow_html=True)
    with tc:
    ...
    persona = st.selectbox("Persona", ["🎓 University Professor","👩‍🏫 School Teacher","🧒 Child-Friendly","🔬 Scientist","📊 Analytical","📝 Exam-Oriented","💪 Motivational"], label_visibility="collapsed")
...
# HERO
st.markdown(f"""<div class="hero"><div class="eyebrow">✦ Powered by Google Gemini 2.5 Flash</div><div class="hero-title">Learn Smarter.<br>Not Harder.</div><div class="hero-sub">Transform any topic or document into notes, flashcards, quizzes and more — in seconds.</div><div class="hero-stats"><div><div class="stat-val">10+</div><div class="stat-lbl">AI Features</div></div><div><div class="stat-val">∞</div><div class="stat-lbl">Topics</div></div><div><div class="stat-val">Free</div><div class="stat-lbl">Forever</div></div></div></div><div class="fpills"><span class="fp">📝 Notes</span><span class="fp">🎴 Flashcards</span><span class="fp">❓ Quiz</span><span class="fp">🧪 Feynman</span><span class="fp">🤖 Tutor</span><span class="fp">📅 Study Plan</span><span class="fp">🎓 Exam Mode</span><span class="fp">🧠 Mind Map</span><span class="fp">💡 Mnemonics</span><span class="fp">⚡ TL;DR</span><span class="fp">👶 ELI5</span><span class="fp">⏱ Pomodoro</span></div>""", unsafe_allow_html=True)

# STEP 1
st.markdown(f"""<div class="sec"><div class="sec-n">1</div><div><div class="sec-title">Enter Topic or Upload File</div><div class="sec-sub">Type any subject, paste notes, or upload PDF / DOCX / TXT</div></div></div>""", unsafe_allow_html=True)
...
manual = st.text_area("", height=110, label_visibility="collapsed", placeholder="e.g. Photosynthesis · Newton's Laws · French Revolution · Machine Learning...")
...
# STEP 2
st.markdown(f"""<div class="sec"><div class="sec-n">2</div><div><div class="sec-title">Read & Learn</div><div class="sec-sub">Start with Notes — then explore summaries, concepts and memory aids</div></div></div>""", unsafe_allow_html=True)
...
if st.session_state.notes_content:
    with st.expander(f"📘 {st.session_state.notes_heading}", expanded=True):
        ...
        st.download_button("📥 Download Notes", data=st.session_state.notes_content, file_name=f"Notes_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", mime="text/plain")
...
if st.session_state.generated_output and st.session_state.generated_heading in S2:
    with st.expander(f"📄 {st.session_state.generated_heading}", expanded=True):
        ...
        st.download_button("📥 Download", data=st.session_state.generated_output, file_name=f"{st.session_state.generated_heading}.txt", mime="text/plain", key="dl_s2")

# STEP 3
if st.session_state.notes_content:
    st.markdown(f"""<div class="sec"><div class="sec-n">3</div><div><div class="sec-title">Test Your Knowledge</div><div class="sec-sub">Flashcards, Quiz, Reflection, Feynman Check & your AI Tutor</div></div></div>""", unsafe_allow_html=True)
            ...
            if u_ans:
                if st.button("📝 Submit Quiz", use_container_width=True, type="primary"):
                    if None in u_ans:
                        ...
                        score = sum(1 for i in range(len(c_ans)) if u_ans[i] and re.match(r'^' + c_ans[i], u_ans[i].strip(), re.IGNORECASE))
    ...
    with tab3:
        ...
        if st.session_state.generated_heading in ["Reflection","Exam Mode"] and st.session_state.generated_output:
            st.markdown(st.session_state.generated_output)
            st.download_button("📥 Download", data=st.session_state.generated_output, file_name=f"{st.session_state.generated_heading}.txt", mime="text/plain", key="dl_ref")
        st.markdown("---")
        st.markdown("#### 🧪 Feynman Technique Checker")
        st.caption("Explain the topic in your own words. AI scores your understanding.")
        fi = st.text_area("Your explanation:", height=120, key="feynman_ta", placeholder="In simple terms, this topic is about...")
    ...
    st.markdown(f"""<div class="sec"><div class="sec-n">4</div><div><div class="sec-title">Exam Mode</div><div class="sec-sub">Full simulated exam — MCQs, fill in blanks, short answers all in one</div></div></div>""", unsafe_allow_html=True)
    ...
    if st.session_state.generated_heading == "Exam Mode" and st.session_state.generated_output:
        st.markdown(st.session_state.generated_output)
        st.download_button("📥 Download Exam Paper", data=st.session_state.generated_output, file_name=f"Exam_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", mime="text/plain")
else:
    st.markdown(f"""<div class="empty-state"><div style="font-size:2.8rem;margin-bottom:0.8rem;">📖</div><div style="font-size:0.95rem;font-weight:700;color:{TEXT2};">Generate Notes first to unlock all testing features</div><div style="font-size:0.8rem;color:{TEXT3};margin-top:0.3rem;">Enter a topic above and click 📝 Generate Notes</div></div>""", unsafe_allow_html=True)

if st.session_state.history:
    st.markdown(f"""<div class="sec"><div class="sec-n" style="background:linear-gradient(135deg,#0f766e,#0891b2);">📜</div><div><div class="sec-title">Session History</div><div class="sec-sub">All generated content from this session</div></div></div>""", unsafe_allow_html=True)
    for i, item in enumerate(reversed(st.session_state.history)):
        with st.expander(f"**{item['format']}** — {item['topic']} | {item['ts']}"):
            st.markdown(item.get("output",""))
            st.download_button("📥 Download", data=item.get("output",""), file_name=f"{item['format']}_{i}.txt", mime="text/plain", key=f"hl_{i}")