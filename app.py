...
if dark:
    ...
    FC_BG = "#0a1428"
    PROG_BG = "#1a2e50"
    SETUP = "linear-gradient(135deg,#060d1e,#091428)"
    STAT_V = "#60a5fa"
    EYEBROW = "#2a5898"
else:
    BG = "#f0f4ff"
    ...
    FC_BG = "#f5f8ff"
    PROG_BG = "#d0daf0"
    SETUP = "linear-gradient(135deg,#dce8ff,#eef3ff)"
    STAT_V = "#2563eb"
    EYEBROW = "#4a78cc"
...
def quota_ui():
    ...
    with st.expander("🔑 Add a free key to continue", expanded=True):
        st.markdown(
            "**Get free key →** [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey)"
        )
        nk = st.text_input(
            "Paste key:",
            type="password",
...
def read_pdf(f):
    return "\n".join(
        [p.extract_text() for p in PyPDF2.PdfReader(f).pages if p.extract_text()]
    )
...
def gen_heading(txt):
    r = generate(
        f"Short heading, max 7 words, no quotes, no punctuation.\nInput: {txt}", 0.2
    )
...
def run_gen(fmt, content, manual, difficulty, persona, creativity, save_as="output"):
    ...
    if save_as == "notes":
        ...
        st.session_state.notes_heading = gen_heading(
            manual if manual.strip() else "Document"
        )
...
def show_setup():
    ...
    st.markdown(
        f"""
    <div style="max-width:620px;margin:1rem auto;padding:0 1rem;">
    <div style="text-align:center;padding:1rem 0 1.5rem;">
        <div style="font-size:2.5rem;">🧠</div>
        <div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:1.5rem;font-weight:800;color:{TEXT};">
            LearnFlow <span style="color:{ACCENT};">AI</span></div>
        <div style="font-family:'Space Mono',monospace;font-size:0.6rem;color:{TEXT3};letter-spacing:0.15em;margin-top:0.2rem;">
            STUDY COMPANION</div>
    </div>
    <div class="setup-card">
        <div class="setup-title">Connect your free AI key</div>
        <div class="setup-sub">
        ...
        <div class="steps-grid">
            ...
            <div class="step-box"><div class="step-n">04</div><div class="step-t">Paste below and go!</div></div>
        </div>
        <div class="tip-box">⚡ Keys look like: <strong>AIzaSyAbc123...</strong> (39 characters)</div>
    </div>
    </div>
    """,
        unsafe_allow_html=True,
    ...
    with mc:
        ...
        <div style="text-align:center;font-size:0.76rem;color:{TEXT3};margin-bottom:1.2rem;">
        ...
        <div class="privacy-box">
            🔒 <strong>Your privacy:</strong> Key stored only in browser session memory.
            Never sent to our servers. Auto-deleted when you close this tab.
        </div>
        <br>
        <div style="font-size:0.78rem;font-weight:700;color:{TEXT2};margin-bottom:0.5rem;">🎓 Everything you unlock:</div>
        <div class="fpills">
            <span class="fp">📝 Notes</span><span class="fp">🎴 Flashcards</span>
            <span class="fp">❓ Quiz</span><span class="fp">🧪 Feynman</span>
            <span class="fp">🤖 Tutor</span><span class="fp">📅 Study Plan</span>
            <span class="fp">🎓 Exam Mode</span><span class="fp">🧠 Mind Map</span>
            <span class="fp">💡 Mnemonics</span><span class="fp">⚡ TL;DR</span>
            <span class="fp">👶 ELI5</span><span class="fp">⏱ Pomodoro</span>
            <span class="fp">🌙 Dark/Light</span>
        </div>
        """,
            unsafe_allow_html=True,
        )
...
<div class="hero">
    ...
    <div class="hero-title">Learn Smarter.<br>Not Harder.</div>
...
st.markdown(
    f"""<div class="sec"><div class="sec-n">1</div>
<div><div class="sec-title">Enter Topic or Upload File</div>
<div class="sec-sub">Type any subject, paste notes, or upload PDF / DOCX / TXT</div></div></div>""",
    unsafe_allow_html=True,
)
...
st.markdown(
    f"""<div class="sec"><div class="sec-n">2</div>
<div><div class="sec-title">Read & Learn</div>
<div class="sec-sub">Start with Notes — then explore summaries, concepts and memory aids</div></div></div>""",
    unsafe_allow_html=True,
)
...
if st.session_state.notes_content:
    st.markdown(
        f"""<div class="sec"><div class="sec-n">3</div>
    <div><div class="sec-title">Test Your Knowledge</div>
    <div class="sec-sub">Flashcards, Quiz, Reflection, Feynman Check & your AI Tutor</div></div></div>""",
        unsafe_allow_html=True,
    )

    tab1, tab2, tab3, tab4 = st.tabs(
        ["🎴 Flashcards", "❓ Quiz", "🤔 Reflect & Feynman", "🤖 AI Tutor"]
    )
        ...
        if st.session_state.generated_heading == "Quiz" and st.session_state.generated_output:
            ...
            for blk in blocks:
                ...
                if opts and qtxt and len(opts) >= 2:
                    ...
                    sel = st.radio(
                        "", opts, key=f"q_{qi}", index=None, label_visibility="collapsed"
                    )
                    u_ans.append(sel)
                    if cline:
                ...
                if st.button("📝 Submit Quiz", use_container_width=True, type="primary"):
                    if None in u_ans:
                        ...
                        score = sum(
                            ...
                            if u_ans[i]
                            and re.match(r"^" + c_ans[i], u_ans[i].strip(), re.IGNORECASE)
                        )
    ...
    with tab4:
        st.caption(
            "AI asks deep Socratic questions — challenges real understanding, not just memory."
        )
        ...
        for msg in st.session_state.tutor_history:
            if msg["role"] == "tutor":
                st.markdown(
                    f'<div class="tutor-q">🤖 <strong>Tutor:</strong> {msg["msg"]}</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<div class="tutor-a">👤 <strong>You:</strong> {msg["msg"]}</div>',
                    unsafe_allow_html=True,
                )
    ...
    # ── STEP 4 ──
    st.markdown(
        f"""<div class="sec"><div class="sec-n">4</div>
    <div><div class="sec-title">Exam Mode</div>
    <div class="sec-sub">Full simulated exam — MCQs, fill in blanks, short answers all in one</div></div></div>""",
        unsafe_allow_html=True,
    )
...
if st.session_state.history:
    ...
    <div class="sec-sub">All generated content from this session</div></div></div>""",
        unsafe_allow_html=True,
    )
